...
var api
## Whether Hearthhold was the game being played when setup() ran (read by the tests).
var setup_saw_game := false
...
func setup(mod_api) -> void:
	...
	setup_saw_game = api.is_game()  # see vanilla's music_setup_saw_game
	api.register_music("hearth", "music/hearth.ogg", {"volume": 0.9,
		"attribution": "Quarrowen placeholder, generated by tools/generate_music.py (CC0)"})
	# A valley with its light out gets vanilla's night music whatever the clock says - cold and sparse
	# suits it - and its own warm track from the moment the hearth is lit.
	# Was a five-second timer restating the same answer. Lighting the hearth already switches the track
	# itself (see _light_hearth), so the only other thing that can change it is the clock turning over.
	# (2026-09-21)
	var choose := func(): api.play_music(null, "hearth" if api.storage.get("hearth_lit", false) else "vanilla:night", {"fade": 6.0})
	api.on("time_changed", func(_ev): choose.call())
	choose.call()
	...
	# Morning after the first night, which is chapter one finished. Was a five-second timer checking
	# whether the clock had wandered into a narrow window - which it could miss, and which fired
	# twelve times an hour to say no. Dawn is an event now. (2026-09-21)
	api.on("time_changed", func(ev):
		if ev.phase == "dawn" and api.storage.get("hearth_lit", false) and not api.storage.get("seen_morning", false):
...
func _light_hearth(player, pos: Vector3i) -> void:
	...
	# The valley's own music, from the moment its light comes back. Vanilla's day and night keep playing
	# until then, which is the right shape: the story takes over when the story starts.
	api.play_music(null, "hearth", {"fade": 4.0})
