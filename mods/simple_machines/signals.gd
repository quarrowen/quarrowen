...
func setup(mod_api, materials: Dictionary) -> void:
	...
	api.register_block("quickstone", {"group": "Stone", "display_name": "Quickstone", "textures": "textures/quickstone.png",
		"light": 4, "hardness": 4.5, "tier": 1, "tool": "pickaxe", "drops": "simple_machines:quickdust_item",
		"sounds": sounds})
	...
	for form in [["quickdust", "textures/quickdust.png", 0], ["quickdust_lit", "textures/quickdust_lit.png", 7]]:
		api.register_block(form[0], {"group": "Mechanism", "display_name": "Quickdust", "textures": form[1], "render": "plant",
			...
			"drops": "simple_machines:quickdust_item", "sounds": sounds})
	...
	api.register_block("lever", {"group": "Mechanism", "display_name": "Lever", "textures": "textures/lever.png", "render": "plant",
		"solid": false, "interactive": true, "hardness": 0.5, "support": "solid", "drops": "simple_machines:lever",
		"sounds": sounds})
	...
	api.register_block("quicklamp", {"group": "Light", "display_name": "Quicklamp", "textures": "textures/quicklamp.png",
		"hardness": 1.0, "drops": "simple_machines:quicklamp", "sounds": sounds})
	api.register_block("quicklamp_lit", {"group": "Light", "display_name": "Quicklamp", "textures": "textures/quicklamp_lit.png",
		"light": 14, "hardness": 1.0, "drops": "simple_machines:quicklamp", "placeable": false, "sounds": sounds})

	# **`require_block`, and this mod's own names.** These ids are kept in a table and written into the
	# world, and `block()` answers -1 for "not installed" - which becomes 65535 as a u16, and 65535 is
	# UNLOADED, so the symptom is a world that reads as absent rather than a registration that failed
	# (CLAUDE.md). When these blocks moved here from `base` the prefix was still "base:", every lookup
	# answered -1, and the lever stopped lighting the dust. `require_block` says so at load instead.
	for block_name in ["quickstone", "quickdust", "quickdust_lit", "lever", "quicklamp", "quicklamp_lit"]:
		ids[block_name] = api.require_block(block_name)

	api.register_recipe({"simple_machines:quickdust_item": 1}, "simple_machines:quickdust", 1, {})
	api.register_recipe({"base:planks": 1, "base:stick": 1}, "simple_machines:lever", 1, {})
	api.register_recipe({"base:glass": 4, "simple_machines:quickdust_item": 4, "base:iron_ingot": 1}, "simple_machines:quicklamp", 1,
