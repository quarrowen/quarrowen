...
GDScript: `api.reloading := false  (property)`
...
### `api.register_ambience`

GDScript: `api.register_ambience(options: Dictionary) -> String`

JavaScript: `api.registerAmbience(options: { sound: string; every?: [number, number]; sky?: boolean; depth?: [number, number]; biome?: string | string[]; near?: string | string[]; radius?: number; chance?: number; volume?: number; pitch?: number; }): string`
...
### `api.set_default_role`

GDScript: `api.set_default_role(role_name: String) -> String`

JavaScript: `api.setDefaultRole(role: string): string`
...
GDScript: `api.register_weather(weather_name: String, def: Dictionary) -> int`

JavaScript: `api.registerWeather(weatherName, def)`
...
GDScript: `api.set_weather(weather_name: String, options := {}) -> void`

JavaScript: `api.setWeather(weatherName, options)`

Starts weather for everyone, or stops it with "". `seconds` of 0 leaves it until something changes it.

### `api.get_weather`

GDScript: `api.get_weather() -> Dictionary`

JavaScript: `api.getWeather()`
...
GDScript: `api.register_music(track_name: String, file: String, options := {}) -> int`

JavaScript: `api.registerMusic(name: string, file: string, options: { attribution: string; volume?: number; loop?: boolean }): number`
...
GDScript: `api.play_music(player, track_name: String, options := {}) -> void`

JavaScript: `api.playMusic(player: Player | null, name: string, options?: { fade?: number; restart?: boolean }): void`
...
GDScript: `api.stop_music(player, options := {}) -> void`

JavaScript: `api.stopMusic(player: Player | null, options?: { fade?: number }): void`
...
JavaScript: `api.setMapMarker(player, markerId, marker)`
...
### `api.clear_map_marker`

GDScript: `api.clear_map_marker(player, marker_id: String) -> void`

JavaScript: `api.clearMapMarker(player, markerId)`

Takes a marker off a player's map.

### `api.set_world_marker`

GDScript: `api.set_world_marker(marker_id: String, marker: Dictionary) -> void`

JavaScript: `api.setWorldMarker(markerId, marker)`
...
### `api.clear_world_marker`

GDScript: `api.clear_world_marker(marker_id: String) -> void`

JavaScript: `api.clearWorldMarker(markerId)`

Takes a marker off everyone's map.

### `api.open_bag`

GDScript: `api.open_bag(player, slot: int) -> bool`

JavaScript: `api.openBag(player, slot)`
...
GDScript: `api.shared_store(store_name: String, container_type: String) -> bool`

JavaScript: `api.sharedStore(storeName, containerType)`
...
GDScript: `api.open_shared(player, store_name: String) -> bool`

JavaScript: `api.openShared(player, storeName)`
...
GDScript: `api.get_shared(store_name: String)`

JavaScript: `api.getShared(storeName)`
...
GDScript: `api.play_decal(position: Vector3, normal := Vector3i.UP, look := {}, realm_id := "") -> void`

JavaScript: `api.playDecal(position, normal, look, realmId)`
...
GDScript: `api.screen_tint(player, look := {}) -> void`

JavaScript: `api.screenTint(player, look)`
...
GDScript: `api.play_beam(from: Vector3, to: Vector3, look := {}, realm_id := "") -> void`
...
GDScript: `api.found_company(company_name: String, founder_id: String) -> int`

JavaScript: `api.foundCompany(companyName, founderId)`
...
GDScript: `api.disband_company(company_id: int) -> bool`

JavaScript: `api.disbandCompany(companyId)`

**See also:** `disband`

### `api.set_company_rank`

GDScript: `api.set_company_rank(company_id: int, player_id: String, rank: String) -> bool`

JavaScript: `api.setCompanyRank(companyId, playerId, rank)`

Ranks are "member", "officer", "owner", least to most.

**See also:** `set_rank`

### `api.remove_from_company`

GDScript: `api.remove_from_company(company_id: int, player_id: String) -> bool`

JavaScript: `api.removeFromCompany(companyId, playerId)`

### `api.company_rank`

GDScript: `api.company_rank(company_id: int, player_id: String) -> String`

JavaScript: `api.companyRank(companyId, playerId)`

**See also:** `rank_of`

### `api.company_at_least`

GDScript: `api.company_at_least(company_id: int, player_id: String, rank: String) -> bool`

JavaScript: `api.companyAtLeast(companyId, playerId, rank)`
...
GDScript: `api.companies_of(who) -> Array`

JavaScript: `api.companiesOf(who)`
...
GDScript: `api.claim_plot(from: Vector3i, to: Vector3i, options := {}) -> int`

JavaScript: `api.claimPlot(from, to, options)`
...
GDScript: `api.release_plot(plot_id: int) -> bool`

JavaScript: `api.releasePlot(plotId)`

**See also:** `release`

### `api.plot_problem`

GDScript: `api.plot_problem() -> String`

JavaScript: `api.plotProblem()`

### `api.plot_at`

GDScript: `api.plot_at(position: Vector3i, realm_id := "") -> Dictionary`

JavaScript: `api.plotAt(position, realmId)`
...
GDScript: `api.may_build(player, position: Vector3i, realm_id := "") -> bool`

JavaScript: `api.mayBuild(player, position, realmId)`
...
GDScript: `api.add_plot_member(plot_id: int, player_id: String) -> bool`

JavaScript: `api.addPlotMember(plotId, playerId)`

**See also:** `add_member`

### `api.remove_plot_member`

GDScript: `api.remove_plot_member(plot_id: int, player_id: String) -> bool`

JavaScript: `api.removePlotMember(plotId, playerId)`

**See also:** `remove_member`

### `api.plots_of`

GDScript: `api.plots_of(who) -> Array`

JavaScript: `api.plotsOf(who)`
...
GDScript: `api.register_objective(objective_name: String, def: Dictionary) -> bool`

JavaScript: `api.registerObjective(objectiveName, def)`
...
GDScript: `api.give_objective(player, objective_name: String) -> bool`

JavaScript: `api.giveObjective(player, objectiveName)`
...
### `api.advance_objective`

GDScript: `api.advance_objective(player, objective_name: String, amount := 1) -> bool`

JavaScript: `api.advanceObjective(player, objectiveName, amount)`
...
### `api.abandon_objective`

GDScript: `api.abandon_objective(player, objective_name: String) -> bool`

JavaScript: `api.abandonObjective(player, objectiveName)`

**See also:** `abandon`, `qualified`

### `api.objectives_of`

GDScript: `api.objectives_of(player) -> Array`

JavaScript: `api.objectivesOf(player)`
...
GDScript: `api.has_objective(player, objective_name: String) -> bool`

JavaScript: `api.hasObjective(player, objectiveName)`

**See also:** `qualified`

### `api.objective_finished`

GDScript: `api.objective_finished(player, objective_name: String) -> int`

JavaScript: `api.objectiveFinished(player, objectiveName)`

How many times they have finished it.

**See also:** `finished`, `qualified`, `register_character`

### `api.register_character`

GDScript: `api.register_character(character_name: String, def: Dictionary) -> bool`

JavaScript: `api.registerCharacter(characterName, def)`
...
GDScript: `api.talk_to(player, character_name: String, options := {}) -> bool`

JavaScript: `api.talkTo(player, characterName, options)`
...
GDScript: `api.has_met(player, character_name: String) -> bool`
...
GDScript: `api.register_shop(shop_name: String, def: Dictionary) -> bool`

JavaScript: `api.registerShop(shopName, def)`
...
GDScript: `api.show_shop(player, shop_name: String) -> bool`

JavaScript: `api.showShop(player, shopName)`
...
GDScript: `api.shop_trade(player, shop_name: String, index: int) -> bool`

JavaScript: `api.shopTrade(player, shopName, index)`
...
GDScript: `api.shop_problem() -> String`

JavaScript: `api.shopProblem()`

### `api.shop_offers`

GDScript: `api.shop_offers(player, shop_name: String) -> Array`

JavaScript: `api.shopOffers(player, shopName)`
...
GDScript: `api.set_nameplate(target, spec := {}) -> bool`

JavaScript: `api.setNameplate(target, spec)`
...
GDScript: `api.nameplate_of(target) -> Dictionary`

JavaScript: `api.nameplateOf(target)`
...
GDScript: `api.clear_nameplate(target) -> bool`

JavaScript: `api.clearNameplate(target)`

**See also:** `float_text`, `follow`

### `api.float_text`

GDScript: `api.float_text(text: String, position: Vector3, options := {}, realm_id := "") -> void`

JavaScript: `api.floatText(text, position, options, realmId)`
...
GDScript: `api.mount(player, entity) -> bool`
...
GDScript: `api.dismount(player, to = null) -> bool`
...
GDScript: `api.riding(player)`

JavaScript: `api.riding(player)`

The entity a player is riding, or null.

### `api.riders_of`

GDScript: `api.riders_of(entity) -> Array`
...
GDScript: `api.tame(entity, player) -> bool`

JavaScript: `api.tame(entity, player)`
...
**See also:** `is_alive`, `key_of`, `play_effect`, `set_plate`, `set_target`

### `api.register_order`

GDScript: `api.register_order(order_name: String, def := {}) -> bool`

JavaScript: `api.registerOrder(orderName, def)`
...
GDScript: `api.order(entity, order_name: String, options := {}) -> bool`

JavaScript: `api.order(entity, orderName, options)`
...
GDScript: `api.order_of(entity) -> String`
...
GDScript: `api.show_orders(player, entity) -> bool`

JavaScript: `api.showOrders(player, entity)`
...
GDScript: `api.register_field(field_name: String, def: Dictionary) -> bool`

JavaScript: `api.registerField(fieldName, def)`
...
GDScript: `api.place_field(field_name: String, position: Vector3, options := {}) -> int`

JavaScript: `api.placeField(fieldName, position, options)`
...
GDScript: `api.clear_field(id: int) -> bool`

JavaScript: `api.clearField(id)`

### `api.fields_at`

GDScript: `api.fields_at(position: Vector3, realm_id := "") -> Array`

JavaScript: `api.fieldsAt(position, realmId)`
...
GDScript: `api.register_condition(condition_name: String, def: Dictionary) -> bool`

JavaScript: `api.registerCondition(conditionName, def)`
...
GDScript: `api.give_condition(target, condition_name: String, options := {}) -> bool`

JavaScript: `api.giveCondition(target, conditionName, options)`
...
GDScript: `api.clear_condition(target, condition_name: String) -> bool`

JavaScript: `api.clearCondition(target, conditionName)`

**See also:** `qualified`

### `api.clear_conditions`

GDScript: `api.clear_conditions(target, only_bad := false) -> int`

JavaScript: `api.clearConditions(target, onlyBad)`
...
GDScript: `api.has_condition(target, condition_name: String) -> bool`

JavaScript: `api.hasCondition(target, conditionName)`

**See also:** `qualified`

### `api.condition_level`

GDScript: `api.condition_level(target, condition_name: String) -> int`

JavaScript: `api.conditionLevel(target, conditionName)`
...
GDScript: `api.conditions_of(target) -> Array`

JavaScript: `api.conditionsOf(target)`
...
GDScript: `api.register_ledger(ledger_name: String, def := {}) -> bool`

JavaScript: `api.registerLedger(ledgerName, def)`
...
GDScript: `api.balance_of(player, ledger_name: String) -> float`

JavaScript: `api.balanceOf(player, ledgerName)`
...
GDScript: `api.add_balance(player, ledger_name: String, amount: float) -> float`

JavaScript: `api.addBalance(player, ledgerName, amount)`
...
GDScript: `api.set_balance(player, ledger_name: String, value: float) -> float`

JavaScript: `api.setBalance(player, ledgerName, value)`

**See also:** `qualified`, `set_value`

### `api.spend_balance`

GDScript: `api.spend_balance(player, ledger_name: String, amount: float) -> bool`

JavaScript: `api.spendBalance(player, ledgerName, amount)`
...
GDScript: `api.level_of(player, ledger_name: String) -> int`

JavaScript: `api.levelOf(player, ledgerName)`
...
GDScript: `api.level_progress(player, ledger_name: String) -> Dictionary`

JavaScript: `api.levelProgress(player, ledgerName)`
...
GDScript: `api.balances_of(player) -> Array`

JavaScript: `api.balancesOf(player)`
...
GDScript: `api.register_modifier(modifier_name: String, def: Dictionary) -> bool`

JavaScript: `api.registerModifier(modifierName, def)`
...
GDScript: `api.apply_modifier(item_data: Dictionary, item_name: String, modifier_name: String, level := 1) -> Dictionary`

JavaScript: `api.applyModifier(itemData, itemName, modifierName, level)`
...
GDScript: `api.modifier_level(item_data: Dictionary, modifier_name: String) -> int`

JavaScript: `api.modifierLevel(itemData, modifierName)`
...
GDScript: `api.modifiers_on(item_data: Dictionary) -> Array`

JavaScript: `api.modifiersOn(itemData)`
...
GDScript: `api.register_liquid(block_name: String, def := {}) -> bool`

JavaScript: `api.registerLiquid(blockName, def)`
...
### `api.register_liquid_meeting`

GDScript: `api.register_liquid_meeting(a_name: String, b_name: String, result_name: String) -> bool`

JavaScript: `api.registerLiquidMeeting(aName, bName, resultName)`
...
GDScript: `api.keep_awake(position: Vector3i, options := {}) -> int`

JavaScript: `api.keepAwake(position, options)`
...
GDScript: `api.let_sleep(claim_id: int) -> bool`

JavaScript: `api.letSleep(claimId)`

Stops keeping it awake.

### `api.wake_claim`

GDScript: `api.wake_claim(claim_id: int) -> bool`

JavaScript: `api.wakeClaim(claimId)`
...
GDScript: `api.set_accepts(node: Dictionary, filter := {}) -> void`
...
GDScript: `api.stop_accepting(node: Dictionary) -> void`
...
GDScript: `api.would_accept(from: Dictionary, item_name: String) -> bool`
...
GDScript: `api.register_drive(unit_name: String) -> bool`

JavaScript: `api.registerDrive(unitName)`
...
GDScript: `api.set_drive(unit_name: String, node: Dictionary, value: float) -> void`

JavaScript: `api.setDrive(unitName, node, value)`
...
GDScript: `api.driven_at(unit_name: String, node: Dictionary) -> float`

JavaScript: `api.drivenAt(unitName, node)`
...
GDScript: `api.drive_jammed(unit_name: String, node: Dictionary) -> bool`

JavaScript: `api.driveJammed(unitName, node)`
...
GDScript: `api.on_driven(unit_name: String, handler: Callable) -> void`

JavaScript: `api.onDriven(unitName, handler)`
...
GDScript: `api.register_unit(unit_name: String) -> bool`

JavaScript: `api.registerUnit(unitName)`
...
GDScript: `api.set_supply(unit_name: String, node: Dictionary, amount: float) -> void`

JavaScript: `api.setSupply(unitName, node, amount)`
...
GDScript: `api.set_demand(unit_name: String, node: Dictionary, amount: float) -> void`

JavaScript: `api.setDemand(unitName, node, amount)`
...
GDScript: `api.received(unit_name: String, node: Dictionary) -> float`

JavaScript: `api.received(unitName, node)`
...
GDScript: `api.on_received(unit_name: String, handler: Callable) -> void`

JavaScript: `api.onReceived(unitName, handler)`
...
### `api.register_link_kind`

GDScript: `api.register_link_kind(kind_name: String, def := {}) -> bool`

JavaScript: `api.registerLinkKind(kindName, def)`
...
GDScript: `api.link(kind_name: String, a: Dictionary, b: Dictionary) -> int`

JavaScript: `api.link(kindName, a, b)`
...
GDScript: `api.link_problem() -> String`

JavaScript: `api.linkProblem()`

Why the last link() was refused.

### `api.link_refused`

GDScript: `api.link_refused(kind_name: String, a: Dictionary, b: Dictionary) -> String`

JavaScript: `api.linkRefused(kindName, a, b)`
...
GDScript: `api.unlink(id: int, why := "removed") -> bool`

JavaScript: `api.unlink(id, why)`
...
GDScript: `api.links_at(position: Vector3i, realm_id := "") -> Array`

JavaScript: `api.linksAt(position, realmId)`
...
GDScript: `api.tag(tag_name: String, names: Array) -> void`

JavaScript: `api.tag(tagName, names)`
...
GDScript: `api.tagged(tag_name: String) -> Array`

JavaScript: `api.tagged(tagName)`
...
GDScript: `api.has_tag(name: String, tag_name: String) -> bool`

JavaScript: `api.hasTag(name, tagName)`
...
GDScript: `api.tags_of(name: String) -> Array`
...
GDScript: `api.register_signal(block_name: String, handler: Callable) -> bool`

JavaScript: `api.registerSignal(blockName, handler)`
...
GDScript: `api.set_signal(position: Vector3i, level: int, realm_id := "") -> void`

JavaScript: `api.setSignal(position, level, realmId)`
...
GDScript: `api.signal_at(position: Vector3i, realm_id := "") -> int`

JavaScript: `api.signalAt(position, realmId)`
...
GDScript: `api.register_instance(kind_name: String, def := {}) -> bool`

JavaScript: `api.registerInstance(kindName, def)`
...
GDScript: `api.open_instance(kind_name: String, options := {}) -> String`

JavaScript: `api.openInstance(kindName, options)`
...
GDScript: `api.enter_instance(player, instance_id: String, position: Vector3) -> bool`

JavaScript: `api.enterInstance(player, instanceId, position)`
...
GDScript: `api.leave_instance(player) -> bool`

JavaScript: `api.leaveInstance(player)`
...
GDScript: `api.close_instance(instance_id: String) -> bool`

JavaScript: `api.closeInstance(instanceId)`
...
JavaScript: `api.instanceOf(player)`
...
GDScript: `api.instance_data(instance_id: String) -> Dictionary`

JavaScript: `api.instanceData(instanceId)`
...
GDScript: `api.add_realm(realm_id: String, options := {}) -> Object`

JavaScript: `api.addRealm(realmId, options)`
...
GDScript: `api.realm_of(player) -> String`

JavaScript: `api.realmOf(player)`
...
GDScript: `api.send_to_realm(player, realm_id: String, position: Vector3) -> bool`

JavaScript: `api.sendToRealm(player, realmId, position)`
...
GDScript: `api.set_wind(degrees: float, strength := 0.5, seconds := 0.0) -> void`
...
GDScript: `api.get_wind() -> Dictionary`

JavaScript: `api.getWind()`
...
### `api.content_conflicts`

GDScript: `api.content_conflicts() -> Array`

JavaScript: `api.contentConflicts()`
...
GDScript: `api.sources_of(item_id: int) -> Array`
...
GDScript: `api.register_source(item_name: String, source: Dictionary) -> bool`

JavaScript: `api.registerSource(itemName, source)`
...
### `api.register_milestone`

GDScript: `api.register_milestone(milestone_name: String, def: Dictionary) -> bool`

JavaScript: `api.registerMilestone(milestoneName, def)`
...
### `api.milestone_reached`

GDScript: `api.milestone_reached(player, milestone_name: String) -> bool`

JavaScript: `api.milestoneReached(player, milestoneName)`
...
GDScript: `api.set_ugc_policy(values: Dictionary) -> void`

JavaScript: `api.setUgcPolicy(values: { enabled?: boolean; accept?: "auto" | "trusted" | "approval" | "off"; kinds?: ("skin" | "accessory" | "model")[]; library?: boolean; max_per_player?: number; max_bytes_per_player?: number; report_hide?: number }): void`
...
GDScript: `api.ugc_list(filter := "approved") -> Array`

JavaScript: `api.ugcList(filter?: "pending" | "reported" | "approved" | "rejected" | "removed" | "all"): Record<string, unknown>[]`
...
GDScript: `api.ugc_get(id: String) -> Dictionary`

JavaScript: `api.ugcGet(id)`

One creation's record, or {}.

### `api.ugc_set_status`

GDScript: `api.ugc_set_status(id: String, status: String, reason := "") -> bool`

JavaScript: `api.ugcSetStatus(id, status, reason)`
...
GDScript: `api.ugc_report(player, id: String, reason := "other", details := "") -> String`

JavaScript: `api.ugcReport(player, id, reason, details)`
...
GDScript: `api.ugc_trust(player_id: String, on := true) -> void`

JavaScript: `api.ugcTrust(playerId, on)`
...
GDScript: `api.ugc_ban(player_id: String, on := true, reason := "") -> void`

JavaScript: `api.ugcBan(playerId, on, reason)`

Stops (or allows again) a player uploading creations; banning also hides their creations.

**See also:** `set_banned`

### `api.register_permission`

GDScript: `api.register_permission(permission: String, description: String, roles := []) -> void`

JavaScript: `api.registerPermission(permission: string, description: string, roles?: string[]): void`
...
GDScript: `api.network_servers() -> Array`

JavaScript: `api.networkServers(): { key: string; name: string; address: string; port: number; hop: boolean; inventory: boolean }[]`
...
GDScript: `api.raycast(origin: Vector3, direction: Vector3, max_distance := 5.0, options := {}) -> Dictionary`

JavaScript: `api.raycast( origin: { x: number; y: number; z: number }, direction: { x: number; y: number; z: number }, maxDistance?: number, options?: { liquids?: boolean }, ): { hit: boolean; position?: { x: number; y: number; z: number }; normal?: { x: number; y: number; z: number }; block?: number }`
...
GDScript: `api.look_direction(player) -> Vector3`

JavaScript: `api.lookDirection(player)`

Where a player is looking, as a unit vector. The same direction the engine uses for their reach.

**See also:** `column`

### `api.set_rejoin_handler`

GDScript: `api.set_rejoin_handler(handler: Callable) -> void`

JavaScript: `api.setRejoinHandler(handler)`
...
GDScript: `api.is_liquid(block: int) -> bool`

JavaScript: `api.isLiquid(block: number): boolean`

Whether a block id is a liquid (water, lava, and anything a mod declares `liquid: true`).

### `api.area_cells`

GDScript: `api.area_cells(rule_name: String, ctx := {}) -> Array`

JavaScript: `api.areaCells(ruleName, ctx)`
...
### `api.register_area_rule`

GDScript: `api.register_area_rule(rule_name: String, chooser: Callable) -> void`

JavaScript: `api.registerAreaRule(ruleName, chooser)`
...
GDScript: `api.area_edit(player, cells: Array, options := {}) -> Dictionary`

JavaScript: `api.areaEdit(player, cells, options)`
...
GDScript: `api.show_area(player, cells: Array, options := {}) -> void`

JavaScript: `api.showArea(player, cells, options)`
...
### `api.add_death_messages`

GDScript: `api.add_death_messages(key: String, lines: Array) -> void`
...
GDScript: `api.register_settings(schema: Dictionary) -> void`

JavaScript: `api.registerSettings(schema)`
...
GDScript: `api.setting(key: String)`

JavaScript: `api.setting(key)`
...
GDScript: `api.is_game() -> bool`

JavaScript: `api.isGame(): boolean`
...
GDScript: `api.game_id() -> String`

JavaScript: `api.gameId()`
...
GDScript: `api.settings() -> Dictionary`
...
GDScript: `api.set_setting(key: String, value) -> String`

JavaScript: `api.setSetting(key, value)`
...
GDScript: `api.static qualified(ref: String, owner: String) -> String`

JavaScript: `api.qualified(ref, owner)`
