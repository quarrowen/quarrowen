#!/usr/bin/env bash
# Builds VoxelCraft.app (a universal app; the native library is Apple silicon only) and zips it for sharing:
#   build/macos/VoxelCraft.app
#   build/macos/VoxelCraft-<version>-mac-arm64.zip
#
...
# `xattr -dr com.apple.quarantine VoxelCraft.app`). Sharing widely needs an Apple Developer ID and
# notarization.
set -euo pipefail
cd "$(dirname "$0")/.."
...
zip="$out/VoxelCraft-$version-mac-arm64.zip"
rm -f "$zip"
