...
Six block models are baked from **Kenney's** CC0 model kits with `tools/bake_model.py`, which scales them
to fit a block and embeds their texture so a client can load them from downloaded bytes alone. The
geometry is his; the baking, the scaling and the use are ours.

| In Quarrowen | From |
|---|---|
| `mods/industry/models/miner.glb` | `hopper-high-square.glb` |
| `mods/industry/models/solar_panel.glb` | `screen-panel-flat.glb` |
| `mods/industry/models/coal_generator.glb` | `machine-fortified.glb` |
| `mods/industry/models/battery.glb` | `machine-window.glb` |
| `mods/industry/models/lamp.glb` | `warning-orange.glb` |
| `mods/guild/models/quest_board.glb` | `signpost.glb` |

The six sources are kept under `assets/` so the bake can be run again; the rest of the kits they came in
were never used and are not in this repository.

## Everything else

The remaining textures, models and sound effects are generated by the scripts in `tools/` and are part
