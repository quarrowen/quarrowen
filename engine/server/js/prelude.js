...
(() => {
  ...
  // Everything in mod_api.gd that has no hand-written entry above, from the table the engine injects
  // as __bindings (generated by tools/bindings_generator.gd from the GDScript signatures).
  //
  // The two APIs are written by hand and drifted to 139 of 262 functions before anybody counted
  // (2026-09-20). This is why a new capability now reaches JavaScript the moment it exists, and why
  // Lua would be a runtime and a prelude rather than another list to keep in step.
  //
  // Hand-written entries win: some rename on the way across, some take friendlier arguments, and none
  // of that should be overruled by a table.
  for (const [name, binding] of Object.entries(globalThis.__bindings ?? {})) {
    if (name in api) continue;
    const callbackAt = binding.args.findIndex((arg) => arg.kind === "callback");
    api[name] = (...args) => {
      // A function has to become an id before it can cross as JSON.
