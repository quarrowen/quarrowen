...
## Rare night creatures, and the capability that announces them (2026-09-23)

*"lets also create some rare mobs like nightstalker, etc. fantasy/mild-horror theme which spawn at
night and not a lot"*, *"should give nice loot"*, *"when such rare mobs spawn, make sure an
announcement is sent over chat or as a overlay or something like that with a poi as well so players
can track n hunt it down"*, *"the marker needs to be on the mob and following it... also the mob
should despawn after a set time, maybe like 10 mins"*, *"display the countdown for the timer to the
players"*.

### What was engine and what was content

Most of it was already there and that is worth recording, because the first instinct was to write a
mod that broadcast a line and put a marker down: `api.broadcast`, `set_world_marker` (map **and**
compass), `entity_natural_spawn`, `entity_spawned` and `entity_death` all existed. The engine even
had the same idea half-built - `GameServer.announce_rare_loot` says a line, makes a noise and leaves
a sparkle that repeats so a rare drop is not lost in the grass.

The one thing a mod could not do cheaply is the **following**. A marker that tracks a wandering
creature needs a handler over every entity every tick; in the engine it is one sweep over a list that
is almost always empty. So the capability is `engine/server/sightings.gd`, opted into by `notable` on
an entity definition - and *which* creatures are rare stays a spawn rule, which is to say a game's
decision.

- `notable: {announce, slain, gone, label, color, minutes}`, or `true` for the defaults.
- The **countdown rides in the marker's label** ("Wisp · 7m"). That was the whole trick: the marker
  is already redrawn every second, already carries a label, and already reaches the client in the map
  reply - so a clock the players can watch cost no new message, no protocol bump and no save change.
  The only client change was drawing the label under the compass pin, which it was not doing.
- Announced **once in a creature's life**, not once per spawn (`data.notable_seen`), or a persistent
  mob would re-announce every time its chunk reloaded.
- Tracked by `"<realm>#<id>"`: entity ids are handed out per realm, so two worlds both have an
  entity 5 and one would have taken the other's marker off the map.

### The subsystem is called `sightings`, and the reason is the ratchet

It was `notable.gd` with `server.notable` first, and `engine/owned.txt` grew two lines - because the
check matches `\.notable\b` and `_server.notable` is a *member* with that name, not a reach into the
shape. That is precisely the false positive `tools/encapsulation.gd` documents as the case it cannot
tell apart. Renaming the subsystem removed the collision rather than baselining it, and the file is
arguably better named for it. Back to 13 entries, unchanged.

### The four

Original, and deliberately from folklore old enough to belong to nobody - a light that leads
travellers off the path, a piper, a thing asleep under a burial mound, a moth at the window. Models
are generated by `tools/box_model.py`, so unlike the thirteen kept from the deleted games these can
be regenerated.

- **Wisp** - `gravity: 0`, fast, cowardly. The hunt is the chase, not the fight.
- **Hollow Piper** - tall, slow, never stops, high knockback resistance.
- **Barrow Warden** - 80 health, immovable, spawns on stony ground: the one you can go looking for.
- **Palemoth** - **cannot hurt anybody**, and is worth more than most things that bite. Four rather
  than three so the youngest child has one to hunt too.

### Loot tables, not `drops` lists

Each has a **loot table** (`"loot": "base:..."`) rather than a flat `drops` array, and the four extra
lines buy a lot: real weights mean the engine knows which entry was the unlikely one, so it sparkles
where it fell, makes a noise, says so in chat, and a run of bad luck eventually pays out (`chance_of`
and the pity rolls in `loot.gd`). Each drops one **guaranteed curio nothing else gives** - Emberheart,
Moonpearl, Hollow Reed, Warden Core - plus a weighted pool of sunstone, cobalt, gold and iron. What
the curios are *for* is a game's business; `base` says they exist.

### Worth knowing

- The spawn rules are in `mods/creative/main.gd` and are **inert there**, like every other spawn rule,
  because creative sets `mob_spawning: false`. Nothing in the shipped content actually spawns one yet
...
### The test camera can now stand somewhere and point at something (user, 2026-09-24)

*"being able to get the test player to the correct spot and view and yaw will be helpful for testing"*
and then *"not just look, but can also teleport near the target right"*. Both are in `tests/screenshot.gd`:

- `--look=firstlight:wick` or `--look=12,70,34` aims at the nearest creature of that type, or a point,
  at 60% of its height - a plate sits above the head and a 1.5 m guide photographed at ankle height is
  a picture of grass.
- `--goto=3` stands 3 m back along the line you were already approaching from first, so the caller
  still chooses the side. Needs the server started with `QW_ADMINS=<--name>`, and says so plainly when
  it never arrives rather than silently photographing wherever it was.

**Both halves run twice: once before `--wait`, and again immediately before the shutter.** That is not
belt-and-braces, it is the whole feature. The first version aimed only at the start, and the picture of
Wick came out as the patch of grass he had been standing on - which is indistinguishable from a model
that is not rendering, and cost most of an evening being investigated as one. Re-aiming showed the yaw
had moved 110 degrees during the wait. The *second* version then re-aimed but did not re-stand, and
because a companion follows you, the picture came out as his hat brim from 0.7 m.

How wrong the guesses had been: hand-picked yaws of `-1.35` against a measured `2.44`. Nearly 180
degrees out, which explains a dozen renders of empty scenery.

The trap on the way in was the one this file already documents: `var back := point - client.state.position`
is a parse error because `client` is untyped, so the *script never loaded* - and a screenshot scene that
does not load never reaches its `get_tree().quit()`, so it presents as a hang rather than an error.

### Wick's lantern was below his feet, and nothing recorded how he was built

The lantern glow was in the `.glb` all along - material 11, emissive, 2.5x strength - and had never
been seen on screen. It was not a rendering problem. **The lantern's boxes had been written relative
to its pivot instead of in model coordinates**, which put the glowing box at y -0.16..0.04, beneath
the ground, and centred on his body rather than in his hand. The symptom was a faint pool of light on
the grass under him, which reads as an emissive that is not working.

What made that expensive is the real finding: **every `.glb` in the tree was generated by a one-off
`box_model.py` command line and then committed, so the only record of how a model was built was the
shell history of whoever built it.** There was no source to read and no way to check the numbers
against intent. `mods/firstlight/models/build.sh` now holds Wick's spec and rebuilds him; the same is
worth doing for the rest as they are touched.

Confirmed on screen at last, in both directions: at night the lantern reads as a lit lamp at his hand,
and by day the nameplate reads "Camera's / Wick" - so `api.tame` putting the owner's name on the plate
works too.

**Still open:** the emission lights the box and casts nothing on the ground. The user likes glows
